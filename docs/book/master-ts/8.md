...
![数组length属性得到数组长度](./imgs/8-1.webp)
...
构造多长的数组是不确定的，需要递归构造，这个我们实现过 [BuildArray](./7#buildarray) ：
...
- 类型参数 Str 是待处理的字符串
- 类型参数 CountArr 是做计数的数组，默认值 [] 代表从 0 开始。
  ...
  Num1 extends Num2
    ...
    : CountArr['length'] extends Num2
      ...
      : CountArr['length'] extends Num1
        ? false
        : GreaterThan<Num1, Num2, [...CounterArr, unknown]>
```
...
![GreaterThan false](./imgs/8-9.webp)



2️⃣ 当 6 和 4 比较时：

![GreaterThan true](./imgs/8-10.webp)
...
![Fibonacci](./imgs/8-11.webp)



## 总结

TypeScript 类型系统没有加减乘除运算符，所以我们**通过数组类型的构造和提取，然后取长度（`length`）的方式来实现数值运算**。
